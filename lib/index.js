...
export const ApolloLinkJWT = ({
   ...
   */
  const refreshTokens = async (_headers) => {
    if (debugMode) {
    ...
    const headers = getDefaultFetchHeaders({ ..._headers, ...fetchHeaders });
   ...
   */
  const SetHeadersLink = setContext(async (_, { headers: _headers }) => {
    if (debugMode) {
    ...
    if ((cachedAccessToken && isJwtExpired(cachedAccessToken)) || (!cachedAccessToken && cachedRefreshToken)) await refreshTokens(headers);
